...
def test_error_decorator_code() -> None:
    for err in Error.all():
        ...
        def bad():
            return "bad"
        assert bad() == [ValidationIssue(err.code, err.name, err.description, "bad")]

def test_warning_decorator_code() -> None:
    for wrr in Warning.all():
        @v.warning(wrr)
        def good():
            return None
        assert good() == []
        ...
        def bad():
            return "bad"
        assert bad() == [ValidationIssue(wrr.code, wrr.name, wrr.description, "bad")]
